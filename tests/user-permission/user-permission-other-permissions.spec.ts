// tests/user-permission/user-permission-other-permissions.spec.ts
...
test.describe('User Permission Other Role', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await test.step('Prerequisite: Login ด้วย Super Admin และไปที่หน้า Permission Settings ของ MD Account', async () => {
      await loginPage.login(process.env.SUPERADMIN2_USERNAME!, process.env.SUPERADMIN2_PASSWORD!);
      await userManagementPage.navigateTo();
      await userManagementPage.searchBar.fill(process.env.MD_USERNAME!);
      await userManagementPage.executiveTab.click();
