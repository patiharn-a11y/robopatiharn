// tests/user-permission/user-permission-admin-permissions.spec.ts
...
test.describe('User Permission Super Admin', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await test.step('Prerequisite: Login ด้วย Super Admin และไปที่หน้า Permission Settings ของ Account ตัวเอง', async () => {
      ...
      await userManagementPage.searchBar.fill(process.env.SUPERADMIN2_USERNAME!);
  ...
  test('TC-020 ตรวจสอบว่า Super Admin ที่ไม่มีสิทธิ์การมองเห็น Portfolio Models จะไม่เห็นเมนูและหน้า Portfolio Models', async () => {
    ...
    await test.step('2. Expected: ไม่เห็นเมนู Portfolio Models ที่แท็บ Sidebar', async () => {
